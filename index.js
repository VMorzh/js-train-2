...
// Задача 3: Напишіть функцію, яка приймає рядок та повертає ціле
//число з цього рядка або повідомлення про помилку.
...
function encodeURLComponent(urlComponent) {
  ...
  const link = encodeURI(urlComponent);
  return link;
}
// Виконуємо функцію з вхідними даними і виводимо результат.
...
function decodeURLComponent(urlComponent) {
  let link = decodeURI(urlComponent);
  return link;
